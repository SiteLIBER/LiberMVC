<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<LiberMvc.PaginatedList<LiberMvc.Models.Blog>>" %>
...
<div class="article_resume">
	<h2>
		<%= Html.ActionLink(art.Titulo, "Details", "Blog", new { id=art.BlogID }, null)%>
	</h2>
	<p class="general_informations">
		<span class="date"><%= art.PublicadoEm.ToShortDateString() %></span>
		<span class="author">
			Por <%: art.Editor.Nome %>
			<%--<%= Html.ActionLink(art.Editor.Nome, "Editor", "Blog", new { id=art.EditorID }, null) %>--%>
		</span>
	</p>
</div>
<% } %>
